...
from util.extra_losses import far_plaque_loss_from_logits, cldice_loss_from_logits
...
def main():
    ...
    parser.add_argument("--model", type=str, default="Echocare", choices=["Echocare", "Echocare_sep_dec","UNet", "UNet++"],
                        help="Model architecture to use: 'Echocare' or 'Echocare_sep_dec' or 'UNet' or 'UNet++'")
    ...
    #new loss args
    parser.add_argument("--lambda_far_plaque", type=float, default=0.1)
    parser.add_argument("--lambda_cldice_vessel", type=float, default=0.1)
    parser.add_argument("--far_plaque_mode", type=str, default="exp", choices=["exp", "linear"])
    parser.add_argument("--far_plaque_dmax", type=float, default=30.0)   # linear용
    parser.add_argument("--far_plaque_sigma", type=float, default=10.0)  # exp용
    parser.add_argument("--cldice_skel_iters", type=int, default=10)
    ...
    for epoch in range(start_epoch, args.train_epochs):
        ...
        writer.add_scalar("Train/Loss_far_plaque", stats["loss_far_plaque"], epoch)
        writer.add_scalar("Train/Loss_far_cldice_vessel", stats["loss_cldice_vessel"], epoch)
...
def train_one_epoch(
    ...
    total_loss_far = AverageMeter()
    total_loss_cldice = AverageMeter()


    loader = zip(loader_l, loader_u, loader_u_mix)

    for i, (
        (x_long, x_trans, m_long, m_trans, y_cls, dist_long, dist_trans),
        ...
        dist_long = dist_long.to(device); dist_trans = dist_trans.to(device)
        ...
        with torch.amp.autocast("cuda", enabled=use_amp, dtype=amp_dtype):
            ...
            # (NEW) FP-oriented extra losses on LABELED only
            # -------------------------
            # FP–plaque 억제: GT plaque에서 멀수록 plaque prob 벌점
            loss_far_plq_L = far_plaque_loss_from_logits(
                segL_x,
                plaque_idx=1,
                dmax=args.far_plaque_dmax,
                sigma=args.far_plaque_sigma,
                mode=args.far_plaque_mode,
                dist_map_bhw=dist_long,    # [B,H,W]
            )

            loss_far_plq_T = far_plaque_loss_from_logits(
                segT_x,
                plaque_idx=1,
                dmax=args.far_plaque_dmax,
                sigma=args.far_plaque_sigma,
                mode=args.far_plaque_mode,
                dist_map_bhw=dist_trans,   # [B,H,W]
            )
            loss_far_plq = (loss_far_plq_L + loss_far_plq_T) / 2.0

            # FP–vessel 억제: vessel이 “길게 연결된 구조”가 되도록 clDice
            loss_cldice_L = cldice_loss_from_logits(
                segL_x, m_long,
                vessel_idx=2,
                skel_iters=args.cldice_skel_iters,
            )
            loss_cldice_T = cldice_loss_from_logits(
                segT_x, m_trans,
                vessel_idx=2,
                skel_iters=args.cldice_skel_iters,
            )
            loss_cldice = (loss_cldice_L + loss_cldice_T) / 2.0
            ...
            loss = (
                ...
                + loss_u_w_fp_seg * 0.5
                + loss_u_s_cls * 0.1
                + args.lambda_far_plaque * loss_far_plq
                + args.lambda_cldice_vessel * loss_cldice
            )
        ...
        total_loss_far.update(loss_far_plq.item())
        total_loss_cldice.update(loss_cldice.item())
    ...
    return {
        ...
        "loss_far_plaque": total_loss_far.avg,
        "loss_cldice_vessel": total_loss_cldice.avg
    }
...
def validate(args, model, valid_loader, device, logger, writer=None, epoch=None):
    ...
    for (x_long, x_trans, m_long, m_trans, y_cls, dist_long, dist_trans) in valid_loader:
        x_long = x_long.to(device)
        x_trans = x_trans.to(device)
        m_long = m_long.to(device)    # (B,H,W)
        m_trans = m_trans.to(device)
        dist_long = dist_long.to(device)
        dist_trans = dist_trans.to(device)
